...
## 1. 목적
DDD(도메인 주도 설계)를 기반으로 D‑PLOG 백엔드를 **모듈러 모놀리스 구조**로 재구성한다.  
단일 Spring Boot 앱을 유지하면서 도메인 경계를 명확히 분리하여 **확장성과 유지보수성**을 높인다.

## 2. 기본 원칙
1) 단일 실행 앱  
`app-api` 모듈만 `@SpringBootApplication`을 보유한다. 나머지 모듈은 라이브러리 모듈로 구성한다.
...
7) `ai-rag`  
Bedrock Knowledge Bases 연동, RAG 응답 생성
...
2) 주요 유스케이스  
지식베이스 검색, 키워드 분석 근거 생성, 리포트 요약문 작성

3) 외부 연동  
AWS Bedrock Knowledge Bases, OpenSearch Serverless, S3

4) 인프라/구조  
Infrastructure: BedrockKbClient, PromptTemplateRepository  
Storage: S3(문서), OpenSearch(벡터), RDS(진단 결과 캐시)  
External: Bedrock KB API
